...
pos_log_var = Conv2DTranspose(3, (3, 3), padding='same', name='pos_log_var')(x)

decoder = tf.keras.Model(latent_inputs, [pos_mean, pos_log_var], name="decoder")
decoder.summary()
...
class VAE(tf.keras.Model):
    ...
    def train_step(self, data):
        with tf.GradientTape() as tape:
            ...
            pos_mean, pos_log_var = self.decoder(z)
            loss_a = float(np.log(2 * np.pi)) + pos_log_var
            loss_m = K.square(pos_mean - data) / K.exp(pos_log_var)
...
dataset_dir = r'C:\Users\Nabaut\Downloads'  # directory where you downloaded the tar files to
...
vae.encoder.save("trained/imagenet_dola_train_"+str(CLASS)+"/encoder")
vae.decoder.save("trained/imagenet_dola_train_"+str(CLASS)+"/decoder")