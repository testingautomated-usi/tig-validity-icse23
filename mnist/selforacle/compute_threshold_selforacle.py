...
if CLASS is None:
    rec_loss_summary = np.load('losses/stocco_rec_losses_MNIST_all_classes.npy')
    calc_and_store_thresholds(rec_loss_summary, "selforacle_thresholds_all_classes")
else:
    rec_loss_summary = np.load('losses/stocco_rec_losses_MNIST_5.npy')
    calc_and_store_thresholds(rec_loss_summary, "selforacle_thresholds_"+str(CLASS))
